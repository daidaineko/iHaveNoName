...
//    NSString* test = [XBMCCommunicator sharedInstance].fulladdress;
    return @"tintin:tintin@192.168.1.13:2081";
}

-(NSString*)createURLBase
{
	NSString *s = [NSString stringWithFormat:@"http://%@/xbmcCmds/xbmcHttp?command=", [self XbmcIPAddress]];
    return s;
}
...
-(BOOL)isSomethingPlaying
{
   ...
   if (NSNotFound == [[theLines objectAtIndex:0] rangeOfString:@"[Nothing Playing]" options:NSCaseInsensitiveSearch].location)
   {
      return YES;
   }
   else
