...
-(void)processNextHTTPRequest
{
    if (currentRequest == nil)
    {
        ...
        NSURLConnection *theConnection=[[NSURLConnection alloc] initWithRequest: [NSURLRequest requestWithURL: [NSString stringWithFormat:@"http://%@/xbmcCmds/xbmcHttp?command=%@", fulladdress,[[currentRequest objectForKey:@"request"] objectForKey:@"cmd"]]] delegate: self startImmediately: YES];
...
- (void)XBMCHTTPCommunicator:(XBMCHTTPCommunicator *)client didReceiveResult:(id)result 
{
    ...
    if ([[currentRequest valueForKey:@"request"] caseInsensitiveCompare:@"GetCurrentlyPlaying"] == NSOrderedSame)
    {
        int i;
        ...
        resultDict = [[NSDictionary alloc] initWithObjectsAndKeys:
                                    [NSNumber numberWithBool:FALSE], @"failure", [currentRequest objectForKey:@"request"], @"request", itemInfo, @"result",nil]; 
    }
...
- (void)XBMCHTTPCommunicator:(XBMCHTTPCommunicator *)client didFailWithErrorCode:(NSNumber *)code message:(NSString *)message
{
    NSDictionary *resultDict = [[NSDictionary alloc] initWithObjectsAndKeys:
                                [NSNumber numberWithBool:TRUE], @"failure", [currentRequest objectForKey:@"request"], @"request", code, @"code", message, @"message",nil];
